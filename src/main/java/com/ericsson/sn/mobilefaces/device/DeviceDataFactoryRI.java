...
/**
 * <p>
 *      Reference implementation of AbstractDeviceDataFactory.
 *      This factory will read device feature information from a xml file, select
 *      current device based on user-agent and save current device object in the
 *      session. Return current device object from session when calling getCurrentDevice().
 * </p>
 *
...
public class DeviceDataFactoryRI extends AbstractDeviceDataFactory{
    ...
    /**
     * @see com.ericsson.sn.mobilefaces.device.AbstractDeviceDataFactory#detectDevice(ServletContext servletContext, HttpServletRequest request)
     */
    @Override
    public void detectDevice(ServletContext servletContext, HttpServletRequest request) {
        if (Log.doAssert(servletContext, "Cannot find Servlet Context when initDevice()", DeviceDataFactoryRI.class)) {
    ...
    /**
     *
     *     @see com.ericsson.sn.mobilefaces.device.AbstractDeviceDataFactory#getCurrentDevice(HttpServletRequest request)
     *
     */
    @Override
    public DeviceData getCurrentDevice(HttpServletRequest request) {
        ...
        DeviceData d = null;
        try {
            d = (DeviceData)session.getAttribute("Device");
        } catch (Exception e){ }
        if (d == null){
            FacesContext context = FacesContext.getCurrentInstance();
        ...
        Log.doAssert(d, "Cannot find current device!", DeviceDataFactoryRI.class);
        return d;
    }

    // Load current device object to session
    private void initSessionDevice(ServletContext servletContext, HttpServletRequest request) {
        HttpSession session = request.getSession();

        Object sessionDevice = null;
        try {
           sessionDevice = session.getAttribute("Device");
        } catch (Exception e){ }
        ...
        if (sessionDevice == null) {
            ...
            DeviceData device = findDevice(servletContext, request);
            if (Log.doAssert(device, "Cannot find device for: " + useragent, DeviceDataFactoryRI.class)) {
                Log.info("Init device for: " + useragent, DeviceDataFactoryRI.class);
    ...
    // Find current device based on user-agent
    private DeviceData findDevice(ServletContext servletContext, HttpServletRequest request) {
        // Read the user-agent from request
        String useragent= request.getHeader("user-agent");

        // Find device define file path from mfaces-config
        SimpleXMLReader config = new SimpleXMLReader(servletContext.getRealPath("/")
        ...
        Collection deviceCollection = propLoader.parse(servletContext.getRealPath("/") + define_path );
        ...
        if (result == null) {
            Log.info("Cannot find device object, using default device!", DeviceDataFactoryRI.class);
            return def;
        } else {
            Log.info("Find device object: " + result.getProperty(DeviceData.DEVICE_NAME), DeviceDataFactoryRI.class);
            return result;
        }
    }
